...
# ═══════════════════════════════════════════════════════════════
# 🔧 FIXED: optimize_and_apply() - Now uses correct method name
# ═══════════════════════════════════════════════════════════════
def optimize_and_apply(db, job, cv_text, profile):
    """Optimizer + Apply Agent: CV tailor karo aur apply karo - FIXED"""
    try:
        from agents.optimizer_agent import OptimizerAgent
        
        # ✅ FIXED: Use correct method name 'optimize_cv_for_job' instead of 'optimize'
        ...
        if apply_email:
            ...
            send_telegram(
                f"📋 <b>Apply manually</b>\n\n"
                f"🏢 {job.get('company')}\n"
                f"💼 {job.get('title')}\n"
                f"🔗 {job.get('apply_url', 'No URL')}\n\n"
                f"CV ready — fill form manually"
            )
            success = False
    ...
    for job in jobs:
        ...
        if score >= AUTO_APPLY_THRESHOLD:
            ...
            if success:
                ...
                log.warning(f"  ❌ Auto-apply failed for {job.get('company')}")
